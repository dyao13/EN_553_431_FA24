...
\textit{Convergence in Probability.} Recall that a sequence of random variables $X_{n}$ converges in probability to $X$ if for all $\delta > 0$, the set 
$$A_{n, \delta} = \{\omega \in \Omega : |X_{n}(\omega) - X(\omega)| \geq \delta\}$$
satisfies
$$P(A_{n, \delta}) \rightarrow 0 \text{ as } n \rightarrow \infty.$$
For all $\delta \in (0, 1]$, the set for which $|g_{n}(\omega) - 0| \geq \delta$ is $\omega \in [j/2^{m}, (j+1)/2^{m}]$. We thus have that 
\begin{align*}
...
Recall that a sequence of random variables $X_{n}$ converges pointwise with probability one to $X$ if there exists a set $A \subseteq \Omega$ with $P(A) = 0$ such that 
$$\forall \omega \in A^{c}, \forall \epsilon > 0, \exists N_{\omega,\epsilon} \in \mathbb{N} \text{ s.t. } |X_{n}(\omega) - X(\omega)| < \epsilon, \forall n > N_{\omega,\epsilon}.$$
